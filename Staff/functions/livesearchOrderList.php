...
if (isset($_POST['name'])) {
    $new = new EmployeeInfo($conn);
    $searchName = $_POST['name'];
    
    // Call searchMenuList only when $searchName is not empty
    if (!empty($searchName)) {
        $searchItem = $new->searchOrderlist($searchName);
        if (!empty($searchItem)) {
            foreach ($searchItem as $row) {
                $cartID = $row["OL_CART_ID"];
                $backgroundColor = '';
                if ($row['STATUS_NAME'] == 'Ready for Pick-up') {
                    $backgroundColor = '#FFDE59';
                } elseif ($row['STATUS_NAME'] == 'To Rate') {
                    $backgroundColor = '#7ED957';
                }elseif ($row['STATUS_NAME'] == 'Preparing') {
                ...
                <div class="top-order">
                        ...
                        <div class="order-status">
                            ...
                            ?>
                            <p class="status <?php echo $statusClass ?>" style="background-color:<?php echo $backgroundColor; ?>"><?php echo $row['STATUS_NAME'] ?></p>
                        </div>
                </div>
